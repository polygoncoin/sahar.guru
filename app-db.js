"use strict"
var MENUAPP = (function(__CATEGORIES, __BRANDS, __PRODUCTS)
{
    //DB details
    const dbName = 'sahar.guru';
    const linkTable = '';
    const brandTable = '';
    const categoryTable = '';
    const productTable = '';

    //classes and ids startes with.
    const categoryCheckboxId = 'cid';
    const categoryCheckboxClass = 'ccl';
    const categoryBrandsClass = 'cb'

    const brandCheckboxId = 'bid';
    const brandCheckboxClass = 'bcl';

    // Product Detail Keys
    const categoryIdsKey = 'categoryIds';
    const brandIdKey = 'brandId';
    const productNameKey = 'productName';
    const productImageKey = 'productImage';

    // Checkbox border colors
    const checkboxPartialColor = 'red';
    const checkboxDefaultColor = '#000000';
    ...
    {
        for (let index in __PRODUCTS) {
            for (let categoryIdsIndex in __PRODUCTS[index][categoryIdsKey]) {
                let categoryIds = __PRODUCTS[index][categoryIdsKey][categoryIdsIndex];
                if (
                ...
                if (typeof categoryHierarchy[categoryId] === "undefined") {
                    ...
                    categoryHierarchy[categoryId]['name'] = __CATEGORIES[categoryId];
                }
                ...
                if (typeof categoryHierarchy[categoryId]['subCategory'][subCategoryId] === "undefined") {
                    ...
                    categoryHierarchy[categoryId]['subCategory'][subCategoryId]['name'] = __CATEGORIES[subCategoryId];
                }
                ...
                categoryHierarchy[categoryId]['subCategory'][subCategoryId]['subCategory'][subSubCategoryId]['name'] = __CATEGORIES[subSubCategoryId];
            }
        }
    },
    hideId = function(id)
        ...
        if (productIds.length === 0) {
            ...
            for (let index  in productIds) {
                ...
                html += `<div class="${tagClass} displayInlineBlock">
                        ...
                        <div class="productName">${__PRODUCTS[index][productNameKey]}</div>
                        <div class="productName">${__BRANDS[__PRODUCTS[index][brandIdKey]]}</div>
                    </div>`;
                if (i%4 === 3) {
                    html += '</div>';
                }
    ...
    setCategoryBreadcrum = function(categoryId, subCategoryId, subSubCategoryId, closeMenu)
    {
        ...
        if (categoryIdOld !== null) {
            ...
            if (
                ...
                breadcrumHtml += ` ${__CATEGORIES[categoryIdOld]}`;
            } else {
                breadcrumHtml += ` <a href="javascript:void(0);" onClick="obj.displayCategoryProducts(${categoryIdOld}, null, null, 'from-breadcrumb');">${__CATEGORIES[categoryIdOld]}</a>`;
            }
        }
        if (subCategoryIdOld !== null) {
            if (
                ...
                breadcrumHtml += ` &gt; <a href="javascript:void(0);" onClick="obj.displayCategoryProducts(${categoryIdOld}, ${subCategoryIdOld}, null, 'from-breadcrumb');">${__CATEGORIES[subCategoryIdOld]}</a>`;
            } else {
                breadcrumHtml += ` &gt; ${__CATEGORIES[subCategoryIdOld]}`;
            }
        }
        if (subSubCategoryIdOld !== null) {
            if (subSubCategoryId === null) {
                breadcrumHtml += ` &gt; <a href="javascript:void(0);" onClick="obj.displayCategoryProducts(${categoryIdOld}, ${subCategoryIdOld}, ${subSubCategoryIdOld}, 'from-breadcrumb');">${__CATEGORIES[subSubCategoryIdOld]}</a>`;
            } else {
                breadcrumHtml += ` &gt; ${__CATEGORIES[subSubCategoryIdOld]}`;
            }
        }
    ...
    setBrandBreadcrum = function(brandId)
    {
        var breadcrumHtml = '';
        if (brandId !== null) {
            breadcrumHtml = `Brand :: ${__BRANDS[brandId]}`;
        }
        if (breadcrumHtml.length > 0) {
            breadcrumHtml += ` <a href="javascript:void(0);" onClick="obj.displayCategoryProducts(null, null, null, false);">X</a>`;
        }
    ...
    displayCategoryProducts = function(categoryId, subCategoryId, subSubCategoryId, closeMenu)
    {
        ...
        let productIds = [];
        for (let index in __PRODUCTS) {
            if (brandIds.indexOf(__PRODUCTS[index][brandIdKey]) === -1) {
                continue;
            }
            if (showAllProducts) {
    ...
    updateBrands = function(categoryId)
    {
        ...
        for (let index in __PRODUCTS) {
            ...
            if (found) {
                let brandId = __PRODUCTS[index][brandIdKey];
        ...
        if (brandCount.length > 0) {
            ...
            var i = 0;
            for (let brandId in __BRANDS) {
                if (typeof brandCount[brandId] !== "undefined") {
                    if (i%4 === 0) {
                        html += '<tr>';
                    }
                    html += `<td width="150" class="brand" align="left" onClick="obj.displayCategoryBrandProducts(${categoryId}, ${brandId})">${__BRANDS[brandId]} (${brandCount[brandId]})</td>`;
                    if (i%4 === 3) {
                        html += '<tr>';
                    }
                    i++;
    ...
    adjustCategoryCheckbox = function(categoryId, subCategoryId, subSubCategoryId)
    {
        ...
        if (id !== categoryCheckboxId) {
            if (allChecked) {
                document.getElementById(id).style.color = checkboxDefaultColor;
            } else if (allUnChecked) {
                document.getElementById(id).style.color = checkboxDefaultColor;
            } else {
                document.getElementById(id).style.color = checkboxPartialColor;    
            }
        }
        switch (true) {
            case subSubCategoryId !== null:
    ...
    adjustBrandCheckbox = function(brandId)
    {
        ...
        if (allChecked) {
            ...
            document.getElementById(brandCheckboxId).style.color = checkboxDefaultColor;
        } else if (allUnChecked) {
            document.getElementById(brandCheckboxId).checked = false;
            document.getElementById(brandCheckboxId).style.color = checkboxDefaultColor;
        } else {
            document.getElementById(brandCheckboxId).checked = true;
            document.getElementById(brandCheckboxId).style.color = checkboxPartialColor;    
        }
    },
        ...
        if (categoryId === null) {
            ...
            for (let index = 0, index_length = checkboxes.length; index < index_length; index++) {
                if (checkboxes[index].id !== categoryCheckboxId) {
                    ...
                    checkboxes[index].style.color = checkboxDefaultColor;
                }
            } 
        } else {
    ...
    brandCheckboxClicked = function(brandId, checked)
    {
        ...
        var checkboxes = document.getElementsByClassName(brandCheckboxClass);
        var color = checkboxDefaultColor;
        for (let index = 0, index_length = checkboxes.length; index < index_length; index++) {
            if (set) {
                checkboxes[index].checked = checked;
                continue;
        ...
        for (let index in __PRODUCTS) {
            ...
            for (let categoryIdsIndex in __PRODUCTS[index][categoryIdsKey]) {
                ...
                if (
                    ...
                    typeof selectedCategories[categoryIds[0]][categoryIds[1]] !== "undefined" &&
                    typeof selectedCategories[categoryIds[0]][categoryIds[1]][categoryIds[2]] !== "undefined" &&
                    brandId == __PRODUCTS[index][brandIdKey] &&
                    brandIds.indexOf(__PRODUCTS[index][brandIdKey]) !== -1
                ) {
                    found = true;
                    break;
                }
    ...
    displayBrandProducts = function(brandId)
    {
        ...
        var productIds = [];
        for (let index in __PRODUCTS) {
            if (brandId == __PRODUCTS[index][brandIdKey]) {
                productIds[index] = index;
            }
        }
        this.generateProductHtml(productIds);
                                ...
                                <td width="200" valign="top">
                                    <table width="100%" cellpadding="0" cellspacing="2">
                                        <tr>
                                            ...
                                            <td class="subMenus" align="left" onClick="obj.displayBrandProducts(${brandId})"><a href="javascript:void(0);">${__BRANDS[brandId]}</a></td>
                                        </tr>
                                    </table>
                                </td>
                            </tr>`;
